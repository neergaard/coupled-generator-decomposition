...
def parse_args(argv):
    ...
    program = dict(
        prog = 'facerecog_make_inverse',
        description = 'Make inverse solutions.'
    )
    arg_subject_id = dict(
...
def facerecog_make_inverse(argv):
    args = parse_args(argv[1:])
    subject_id = getattr(args, 'subject-id')
    bids_root = getattr(args, 'bids-root')
    analysis_root = getattr(args, 'analysis-root')

    pipeline.make_inverse(subject_id, bids_root, analysis_root)

if __name__ == '__main__':
    facerecog_make_inverse(sys.argv)