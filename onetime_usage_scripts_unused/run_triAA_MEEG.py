...
X_test_l2_cat = {}
for m in range(M):
    X_train[modality_names[m]] = torch.load(
        "data/concatenatedData/X_" + modality_names[m] + "_split0.pt"
    )
    X_test[modality_names[m]] = torch.load(
        "data/concatenatedData/X_" + modality_names[m] + "_split1.pt"
    )
    X_train_cat[modality_names[m]] = torch.cat(
        (
            X_train[modality_names[m]][:, 0],
            X_train[modality_names[m]][:, 1],
            X_train[modality_names[m]][:, 2],
        ),
        dim=-1,
    )
    X_test_cat[modality_names[m]] = torch.cat(
        (
            X_test[modality_names[m]][:, 0],
            X_test[modality_names[m]][:, 1],
            X_test[modality_names[m]][:, 2],
        ),
        dim=-1,
    )

    X_train_l2[modality_names[m]] = torch.load(
        "data/concatenatedData/Xf_" + modality_names[m] + "_split0.pt"
    )
    X_test_l2[modality_names[m]] = torch.load(
        "data/concatenatedData/Xf_" + modality_names[m] + "_split1.pt"
    )
    X_train_l2_cat[modality_names[m]] = torch.cat(
        (
            X_train_l2[modality_names[m]][:, 0],
            X_train_l2[modality_names[m]][:, 1],
            X_train_l2[modality_names[m]][:, 2],
        ),
        dim=-1,
    )
    X_test_l2_cat[modality_names[m]] = torch.cat(
        (
            X_test_l2[modality_names[m]][:, 0],
            X_test_l2[modality_names[m]][:, 1],
            X_test_l2[modality_names[m]][:, 2],
        ),
        dim=-1,
    )
...
model_full = TMMSAA.TMMSAA(
    dimensions=(B, L, N, P), num_modalities=M, num_comp=6, model="AA", C_idx=C_idx
)
optimizer = torch.optim.Adam(model_full.parameters(), lr=0.1)
loss_full = TMMSAA_trainer.Optimizationloop(
    model=model_full, X=X_train, Optimizer=optimizer, max_iter=10000, tol=1e-16
)
...
for K in K_eval:
    ...
    model_equalarchetypes = TMMSAA.TMMSAA(
        dimensions=(B, N, P * 3), num_modalities=M, num_comp=K
    )
    optimizer = torch.optim.Adam(model_equalarchetypes.parameters(), lr=0.1)
    loss_equalarchetypes = TMMSAA_trainer.Optimizationloop(
        model=model_equalarchetypes,
...
for m in range(M):
    for c in range(L):
        for k in range(C.shape[1]):
            axs[m, c].bar(
                times,
                torch.mean(S_sort[m, :, k, delims[c] : delims[c + 1]].detach(), dim=0),
                width=(max(times) - min(times)) / len(times),
                bottom=bottom,
            )
            bottom += torch.mean(
