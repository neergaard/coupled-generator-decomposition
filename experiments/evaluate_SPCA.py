    ...
    for k,K in enumerate(num_comps):
        ...
        for outer in range(num_iter_outer):
            ...
            for inner in range(num_iter_inner):
                try:
                    allinner[inner]=np.genfromtxt("data/SPCA_results/loss_"+modeltype+"_K="+str(K)+"_rep_"+str(outer)+"_"+str(inner),delimiter=',')
                except:
                    allinner[inner] = np.nan
            loss[outer] = np.nanmin(allinner,axis=0)
        ax[m,k].imshow(np.nanmean(loss,axis=0),vmin=-40,vmax=10)
