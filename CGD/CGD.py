...
class CGD(torch.nn.Module):
    ...
    *       - indicates other dimensions, e.g., subjects or conditions, across which N and P are equal.
    ...
    def __init__(self, num_comp, X, Xtilde=None, model="SPCA", G_idx=None, lambda1=None, lambda2=None, init=None):
        ...
        if model == "SPCA":
            self.Xsqnorm = torch.zeros(num_modalities, dtype=torch.float)
            self.XtXtilde = torch.zeros((num_modalities, *other_dims, P, torch.sum(G_idx)), dtype=torch.float)
        ...
        if self.model == "AA" or self.model == "DAA":
            ...
            if init is None:
                self.G = torch.nn.Parameter(
                    self.softmaxG(-torch.log(torch.rand((torch.sum(G_idx).int(), num_comp), dtype=torch.float)))
                )
                self.S = torch.nn.Parameter(self.softmaxS(-torch.log(torch.rand(self.S_size, dtype=torch.float))))
            else:
                self.G = init["G"].clone()
                self.S = init["S"].clone()
            ...
            if init is None:
                self.Bp = torch.nn.Parameter(torch.rand((torch.sum(G_idx).int(), num_comp), dtype=torch.float))
                self.Bn = torch.nn.Parameter(torch.rand((torch.sum(G_idx).int(), num_comp), dtype=torch.float))
            else:
    ...
    def get_model_params(self):
        with torch.no_grad():
            if self.model == "AA" or self.model == "DAA":
                ...
                G = Bpsoft - Bnsoft
                S = torch.zeros(self.S_size, dtype=torch.float)
    ...
    def eval_model(self, Xtrain, Xtest, Xtraintilde=None, G_idx=None):
        with torch.no_grad():
            ...
            if self.model == "AA" or self.model == "DAA":
                ...
                XtXtilde = torch.zeros((num_modalities, *other_dims, P, torch.sum(G_idx)), dtype=torch.float)
                for m, key in enumerate(self.keys):
